...
bool Adafruit_ssd1306syp::initialize(int contrast)
{
	//setup the pin mode
...
void Adafruit_ssd1306syp::drawBitmap(int16_t lines, int16_t ofs, uint8_t bitmap[])
// 128/8 = 16
{
  int o = ofs;
	if ((lines >= height()) || ( m_pFramebuffer==0))	return;

	for (int b = 0; b < lines/8; b++)
  {
	  for (int a = 0; a < 128-ofs; a++)
  	  m_pFramebuffer[b*128+a] = pgm_read_byte_near(bitmap+a+o);
	  o += 128;  
	}
}
